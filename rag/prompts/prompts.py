    ...
    return [
        {
            ...
            "kg_id": chunk.get("kg_id"),
            "kg_name": chunk.get("kg_name"),
            "kg_description": chunk.get("kg_description"),
            "kg_node_num": chunk.get("kg_node_num"),
            "kg_edge_num": chunk.get("kg_edge_num"),
            "kg_entity_name": (chunk.get("entity") or {}).get("entity_name"),
            "kg_entity_type": (chunk.get("entity") or {}).get("entity_type"),
            "kg_entity_description": (chunk.get("entity") or {}).get("description"),
        }
...
def kb_prompt(kbinfos, max_tokens, hash_id=False):
    ...
    for i, ck in enumerate(kbinfos["chunks"][:chunks_num]):
        ...
        elif ck.get("source_type") in {"web", "web_knowledge"}:
        ...
        formatted_token_count += num_tokens_from_string(cnt)
        if max_tokens * 0.97 < formatted_token_count:
            logging.warning(f"Not all the retrieval into prompt after formatting: {len(knowledges)}/{kwlg_len}")
            break
        knowledges.append(cnt)

    kbinfos["chunks"] = kbinfos["chunks"][:len(knowledges)]
    doc_ids = {get_value(ck, "doc_id", "document_id") for ck in kbinfos["chunks"]}
    doc_ids.discard(None)
    if "doc_aggs" in kbinfos:
        kbinfos["doc_aggs"] = [
            doc for doc in kbinfos["doc_aggs"]
            if get_value(doc, "doc_id", "document_id") in doc_ids
        ]

export default {
  translation: {
    ...
    login: {
      ...
      forgotPassword: '重置密码',
      forgotPasswordDescription: '请输入邮箱地址以重置密码',
      yourNewPasswordIs: '您的新密码是',
      verificationFailed: '邮箱或姓名不正确',
    ...
    knowledgeDetails: {
      ...
      rerankTip: `非必选项：若不选择 rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 rerank 模型，则混合查询中的向量相似度部分将被 rerank 打分替代。请注意：采用 rerank 模型会非常耗时。如需选用 rerank 模型，建议使用 SaaS 的 rerank 模型服务；如果你倾向使用本地部署的 rerank 模型，请务必确保你使用 docker-compose-gpu.yml 启动 AeroKG。`,
      topK: 'Top-K',
...
`,
    ...
    <p>
      ...
      您只需与<i>'AeroKG'</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
General：实体和关系提取提示来自 GitHub - microsoft/graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
    ...
    chat: {
      ...
      system: '系统提示词',
      systemInitialValue: `你是一个智能助手，请总结知识库的内容来回答问题，请列举文档库中的数据详细回答。当所有知识库内容都与问题无关时，你的回答必须包括“知识库中未找到您要的答案！”这句话。回答需要考虑聊天历史。
        以下是知识库：
        {knowledge}
        以上是知识库。`,
      systemMessage: '请输入',
